...
public class ContentCompare {
	...
	private static String removeSign(String str) {
		...
		for (char item : str.toCharArray())
			/*
			 * if (charReg(item)) { sb.append(item); }
			 */
			sb.append(item);
	...
	private static String similarityResult(double resule) {
		...
		return df.format(resule * 100.0);
	}

	public static String countScore(String strA, String strB) {
		return similarityResult(SimilarDegree(strA, strB));
	}

	static String str1 = "expr 1+1\r\n" + "$((1+1))\r\n" + "config='expr 1+2'\r\n" + "expr $config + 3";
	static String str2 = "expt 1+1\r\n" + "$((1+1))\r\n" + "result='exptdasdasf 1+1'\r\n" + "expt $result + 1";

	public static void main(String[] args) {
		long start = System.currentTimeMillis();
		String result = countScore(str1, str2);
		long end = System.currentTimeMillis();
		System.out.println("相似度： " + result + "，耗时： " + ((end - start) / 1000) + "秒");
	}
